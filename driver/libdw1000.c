...
int dwConfigure(dwDevice_t* dev)
{
	...
	if (dwGetDeviceId(dev) != 0xdeca0130) { //check device ID
...
void dwSetAfterRxAutoSleep(dwDevice_t *dev){
    ...
    dwSpiWrite32(dev, PMSC, PMSC_CTRL1_SUB, pmsc_ctrl1);
}

void dwNotSetAfterRxAutoSleep(dwDevice_t *dev){
    uint32_t pmsc_ctrl1;
    pmsc_ctrl1 = 0x81020738; //Disable ARXSLP,Disable SNPOZE
    dwSpiWrite32(dev, PMSC, PMSC_CTRL1_SUB, pmsc_ctrl1);
}

void dwSetPreambleDectTimeOut(dwDevice_t *dev, uint16_t timeout_PAC_Size){
	dwSpiWrite16(dev, DRX_TUNE, DRX_PRETOC_SUB, timeout_PAC_Size);
}

void dwSetSnozeTime(dwDevice_t *dev, uint8_t timeout_snoze){
	dwSpiWrite8(dev, PMSC, PMSC_SNOZT_SUB, timeout_snoze);
}

void dwSetSleepEnAndTime(dwDevice_t *dev){
	uint32_t aon_cfg0;
	aon_cfg0 = 0x02B81FEF; //EN Sleep, Sleep Counter Time = 300s
	dwSpiWrite32(dev, AON, AON_CFG0_SUB, aon_cfg0);
}

void dwSetAONWakeUpConfig(dwDevice_t *dev){
	uint16_t aon_wcfg;
	//wake turn on receiver
...
void dwLowPowerListenMode(dwDevice_t *dev, uint32_t listen_timeout_us, uint32_t timeout_snoze_us){
	...
	dwSetSleepEnAndTime(dev);
	dwSetAONWakeUpConfig(dev);
	dwClearInterrupts(dev);
	dwInterruptOnReceived(dev, true);
	dwWriteSystemEventMaskRegister(dev);
	dwNewReceive(dev);
...
void dwSetAutoAck(dwDevice_t *dev, bool val){
	setBit(dev->syscfg, LEN_SYS_CFG, AUTOACK_BIT, val);
}

void dwSetAckAndRespTime(dwDevice_t *dev, uint32_t ack_time_us, uint32_t resp_time_us){
